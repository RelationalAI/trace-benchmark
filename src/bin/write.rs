...
fn main() {
    let input_path: String = std::env::args()
        .nth(1)
        .unwrap()
        .parse()
    ...
    timely::execute_from_args(std::env::args().skip(3), move |worker| {
        ...
        let measurements = File::open(&input_path).expect("failed to open measurements file");
        ...
        loop {
            match reader.read_line(&mut buf) {
                ...
                Ok(_n) => {
                    ...
                    let r = buf.parse::<R>().expect("failed to parse measurement");
